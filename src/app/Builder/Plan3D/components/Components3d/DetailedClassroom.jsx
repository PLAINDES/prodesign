...
// ✅ USAR DIMENSIONES CUSTOM SI ESTÁN DISPONIBLES
    const baseWidth = customRoofWidth && customRoofWidth > 0
        ? customRoofWidth
        : techoADosAguas.esVertical
          ? Math.max(aulaData.width + 1.2, 0.5)
          : Math.max(aulaData.depth + 1.2, 0.5);
const altura =
        techoADosAguas.alturaCumbrera;
...
// ✅ USAR PROFUNDIDAD CUSTOM SI ESTÁ DISPONIBLE
    const depth = customRoofDepth && customRoofDepth > 0
        ? customRoofDepth
        : techoADosAguas.esVertical
          ? Math.max(aulaData.depth + 1.2, 0.5)
          : Math.max(aulaData.width + 1.2, 0.5),
